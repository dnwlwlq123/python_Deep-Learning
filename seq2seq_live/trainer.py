...
class Trainer:
    ...
    def train(self, train_loader, valid_loader=None, num_epochs=10, print_every=1, evaluate_every=1,
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if valid_loader and epoch % evaluate_every == 0:
                ...
                if val_loss < self.best_val_loss:
                    ...
                    print(
                        f'Best model saved with validation loss: {val_loss:.4f}, validation {evaluate_metric}: {val_metric:.4f}')
                    val_counter = 0
                else:
                    val_counter += 1
        ...
        with torch.no_grad():
            for source, target in valid_loader:
                ...
                if evaluate_metric == 'accuracy':
                    ...
                    pred = torch.argmax(output, dim=-1)
                    bleu_sum = sum(
                        [bleu(pred_sent, target_sent) for pred_sent, target_sent in zip(pred, target)]) / len(target)
                    output = output[:, 1:].reshape(-1, output_dim)
                    target = target[:, 1:].reshape(-1)
                loss = self.criterion(output, target)
    ...
    def inference(self, input_sequence, vocab):
        ...
        output_sequence = [vocab.index2word[idx] if idx in vocab.index2word else vocab.OOV for idx in
                           outputs]
