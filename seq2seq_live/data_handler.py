...
def indices_to_one_hot(batch, vocab_size):
    ...
    assert one_hot.size() == (batch_size, seq_length,
                              vocab_size), f"Output should have shape {(batch_size, seq_length, vocab_size)}, but got {one_hot.size()}"
...
def create_integer_sequence_dataset(num_samples=1000, max_input_len=10, max_target_len=15, vocab_size=20, batch_size=32,
    ...
    dataloaders = [DataLoader(dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn) for dataset in
                   datasets]
...
def create_lang_pair(data_file, batch_size=32, train_valid_test=(0.8, 0.1, 0.1)):
    ...
    dataloaders = [DataLoader(dataset, batch_size=batch_size, shuffle=True,
                              collate_fn=lambda x: collate_fn_language(x, source_vocab, target_vocab)) for dataset in
                   datasets]
...
def collate_fn_language(batch, source_vocab, target_vocab):
    ...
    target_seqs_indices = []
    for seq in target_seqs:
        seq_indices = [target_vocab.sos_idx] + [target_vocab.word_to_index(word) for word in seq] + [
