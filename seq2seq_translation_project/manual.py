...
class RNNCellManual(nn.Module):
    ...
    def forward(self, x_t, h_t):
        ...
        assert h_t.size(0) == batch_size, (f'0th dimention of output of RNNManualCell is expected to be {batch_size},'
                                           f'got {h_t.size(0)}')
        assert h_t.size(1) == self.hidden_dim (f'1th dimention of output of RNNManualCell is expected to be {self.hidden_dim},'
                                           f'got {h_t.size(1)}')
        return h_t
    def initialize(self):
...
class LSTMCellManual(nn.Module):
    def __init__(self, input_dim, hidden_dim):
        ...
        self.i2i = nn.Linear(input_dim, input_dim)
        self.h2i = nn.Linear(hidden_dim, input_dim)
    ...
    def forward(self, x_t, h_t, c_t):
        ...
        g_t = torch.than(self.i2g(x_t) + self.h2g(h_t))
        o_t = torch.sigmoid(self.i2o(x_t) + self.h2o(h_t))

        c_t = f_t * c_t + i_t * g_t
        h_t = o_t * torch.than(c_t)
