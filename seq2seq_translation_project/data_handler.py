...
class Vocabulary:
    ...
    def __init__(self, word_count, coverage = 0.999):
        ...
        for idx, (word, freq) in enumerate([(e, 0) for e in
            Vocabulary.SPECIAL_TOKENS] + word_freq_list):
            s += freq
            if s > coverage * total:
                break
            word2idx[word] = idx
...
def parse_file(file_path, train_valid_test_ratio = (0.8, 0.1, 0.1), batch_size = 32,):
    ...
    dataloaders = [DataLoader(dataset,
                              ...
                              shuffle = True,
                              collate_fn = lambda x: preprocessing(x,
...
def preprocessing(batch, source_vocab, target_vocab):
    ...
    for source_seq in sources:
        source_seqs.append(source_seq + [source_vocab.
        EOS_IDX])
    for target_seq in targets:
        target_seqs.append([target_vocab.SOS_IDX] +
        target_seq + [target_vocab.EOS_IDX])
    ...
    for idx, seq in enumerate(target_seqs):
        seq = seq + [source_vocab.PAD_IDX] *(target_max_length - len(seq))
